...
public class MultiTouchHandler implements TouchHandler {
    ...
    public MultiTouchHandler(View view,float scaleX, float scaleY) {
        Pool.PoolObjecFactory<Input.TouchEvent> factory=new Pool.PoolObjecFactory<Input.TouchEvent>() {
            @Override
        ...
        synchronized (this){
            ...
            for (int i=0;i<MAX_TOUCHPOINTS;i++){
                ...
                if (event.getAction()!= MotionEvent.ACTION_MOVE&& i!=pointerIndex){
                    // if it's an up/down/cancle/ out even, mask the id to see if we should process it for thos touch
                    //point
                    continue;
                }
                switch (action){
